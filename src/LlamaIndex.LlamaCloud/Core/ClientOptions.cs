...
public record struct ClientOptions()
{
    ...
    Lazy<string> _baseUrl = new(() =>
        Environment.GetEnvironmentVariable("LLAMA_CLOUD_BASE_URL") ?? EnvironmentUrl.Production
    );
    ...
    /// <see cref="ILlamaCloudClient.WithRawResponse"/>, don't perform validation until deserialization
    /// is triggered.</para>
    /// </summary>
