...
public sealed record class SheetListPageResponse : JsonModel
{
    ...
    /// The list of items.
    /// </summary>
    public required IReadOnlyList<SheetsJob> Items
    {
        get
        {
            this._rawData.Freeze();
            return this._rawData.GetNotNullStruct<ImmutableArray<SheetsJob>>("items");
        }
        init
        {
            this._rawData.Set<ImmutableArray<SheetsJob>>(
                "items",
...
#pragma warning restore CS8618
    ...
    [SetsRequiredMembers]
    public SheetListPageResponse(IReadOnlyList<SheetsJob> items)
        : this()
    {
        this.Items = items;
    }
