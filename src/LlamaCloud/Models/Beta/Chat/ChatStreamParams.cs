...
public record class ChatStreamParams : ParamsBase
{
    ...
    /// </summary>
    public required IReadOnlyList<string> IndexIds
    {
        get
        {
            this._rawBodyData.Freeze();
            return this._rawBodyData.GetNotNullStruct<ImmutableArray<string>>("index_ids");
        }
        init
        {
...
#pragma warning disable CS8618
    ...
    {
        this.SessionID = chatStreamParams.SessionID;

        this._rawBodyData = new(chatStreamParams._rawBodyData);
    }
    ...
    public override Uri Url(ClientOptions options)
    {
        return new UriBuilder(
            options.BaseUrl.ToString().TrimEnd('/')
                + string.Format("/api/v1/chat/{0}/messages/stream", this.SessionID)
        )
        {
